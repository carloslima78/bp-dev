...
{
    public class StandartPersistence
    {
        private readonly string connectionString;

        #region Constructor

        public StandartPersistence(string connectionString)
        {
        ...
        public IEnumerable<T> GetEntities<T>(CommandType commandType, string sql, object parameters = null)
        {
        ...
        public IEnumerable<T> Select<T>(object criteria = null)
        {
            ...
            return GetEntities<T>(CommandType.Text, sql, properties.AllPairs);
        }
        ...
        private static PropertyContainer ParseProperties<T>(T obj)
        {
            ...
            string.Format("Id{0}", typeName), string.Format("{0}_Id", typeName) };
            ...
            foreach (var property in properties)
            {
                ...
                if (property.IsDefined(typeof(Infra.DataBaseUtils.DapperIgnore), false))
                    continue;

                var name = property.Name;
                var value = typeof(T).GetProperty(property.Name).GetValue(obj, null);

                if (property.IsDefined(typeof(Infra.DataBaseUtils.DapperKey), false) || validKeyNames.Contains(name))
                {
